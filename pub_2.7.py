...
RESET_DATABASE = True    # recreate database from scratch (instead of querying what we've got)
PURGE_LAST_SEASON = False  # no need to recreate the whole database from scratch.  just burn and re-parse the last season's page
...
def get_season(season, overwrite=False):
    ...
    httpdata = urllib2.urlopen(url)
    fh = open(output, "w")
    fh.write(httpdata.read())
    fh.close()
